...
- **Current live state**: backend is running on EC2 right now with `LLM_PROVIDER=bedrock`, confirmed healthy, confirmed answering real requests (including the token-hungry path-generation prompt) correctly through Bedrock.
- **Open decision for the team**: keep `bedrock` as the production default, or revert the default to `groq` with Bedrock kept as a verified, switchable option (one `.env` line either way — both paths are fully working code, this is a preference/cost/latency call, not a technical blocker).
