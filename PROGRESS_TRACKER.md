# PROGRESS TRACKER
Last updated by: Abinivas (Member 2 — ML) at 2026-08-28 (real-data pass on /progress datasets #1-2 of 9, incremental — fixed a real 0-minutes logging bug along the way)
...
## Round 3 — /progress real-data rebuild, incremental (in progress, user reviewing each step)
Doing this one dataset at a time per user direction, not all at once. Progress so far:
- **Dataset #1 (done)**: hero overview card + progress-timeline area chart. `roadmap.percent`/`.currentWeek`/`.weeks`/`.path.goal_text` now drive the "68%" ring, "Continue Week N" button, weeks-completed count, and the timeline chart (cumulative real completion mapped onto however many weeks the roadmap *actually* has, not a fixed 8). Two numbers removed rather than faked ("+8% this month" had no real historical baseline to compute from) — replaced with a real "X of Y steps done" count.
- **Dataset #2 (done)**: weekly-activity hours bar chart. Found and fixed a real bug along the way: every `study_sessions` row ever logged had `minutes: 0` because `roadmap_service.set_task_completion()` called `log_session()` without ever passing a duration. Now logs the completed course's real `duration_hrs` as the time-investment estimate (the only real signal available — no live session timer exists). Added `account_service.get_streak() → daily_minutes_this_week` (7 real per-day sums for the current calendar week). Chart and header stats ("17.4 hrs", "+12% vs avg") now use this — the "vs avg" comparison was dropped (no stored historical baseline) in favor of a real streak-day count.
- **Remaining (7 of 9)**: core skills, skill radar, activity heatmap, milestones timeline, next-actions, recent activity, achievement badges — then `/skills` page's data (separate from `/progress`, ~100% mock, not yet started).

**Confirmed but NOT yet fixed — queued, roughly in priority order:**
1. **`/progress` datasets #3-9 + all of `/skills` page data** — see "Round 3" above for exact status. `roadmap_service.py` already computes real weeks/completion/timestamps/skill_tags, so most of the rest is rebuildable from data that already exists — mainly frontend work, occasionally a small real backend addition (like `daily_minutes_this_week` above) when the exact shape needed doesn't exist yet.
