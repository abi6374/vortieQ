# PROGRESS TRACKER
Last updated by: Abinivas (Member 2 — ML) at 2026-08-28 (web search moved into the planner itself; removed 2 duplicate fake chatbots on /progress + /skills)
...
1. **`/skills` and `/progress` pages' actual DATA** — the fake chatbots are gone, but the underlying datasets are still mostly hardcoded: `/skills` is ~100% mock (skill percentages, gaps, charts), `/progress` has **9 separate hardcoded datasets** (timeline, weekly activity, core skills, radar, heatmap, milestones, next-actions, recent activity, achievements) despite `useRoadmap`/`useStreak` already providing some real data in the same file. Backend reality check: `roadmap_service.py` already computes real weeks/completion/timestamps/skill_tags, so most of this is rebuildable from data that already exists — mainly frontend work, not new backend infra. Not started — this is a big rebuild, sequencing/scope confirmation pending with the user.
