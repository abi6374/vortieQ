...
export default function ResourceItem({ step, onRefresh }) {
  ...
  const difficultyClass = DIFFICULTY_BADGE[difficulty] || 'bg-gray-100 dark:bg-[#18181D] text-gray-700 dark:text-[#CBD5E1] border border-gray-200 dark:border-[#27272F]'

  return (
    <div
      className={`p-4 border rounded-xl transition-colors ${
        isCompleted
          ? 'bg-gray-50 dark:bg-[#0E0E12] border-gray-200 dark:border-[#27272F] opacity-70'
          : 'bg-white dark:bg-[#121216] border-gray-200 dark:border-[#27272F] hover:border-indigo-300 dark:hover:border-[#C9D0D6]'
      }`}
    >
      <div className="flex justify-between items-start gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            ...
            {course.duration_hrs != null && (
              <span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-gray-100 dark:bg-[#18181D] text-gray-600 dark:text-[#C9D0D6] border border-gray-200 dark:border-[#27272F]">
                {course.duration_hrs} hrs
              </span>
            )}
            {status === 'skipped' && (
              <span className="px-2 py-0.5 rounded-full text-[11px] font-medium bg-gray-200 dark:bg-[#18181D] text-gray-500 dark:text-[#A1A1AA]">
                skipped
              </span>
            )}
          </div>
        ...
        <button
          ...
          className="text-xs font-medium text-indigo-600 dark:text-[#C9D0D6] hover:text-indigo-800 dark:hover:text-white shrink-0 cursor-pointer"
        >
          Why this? →
        </button>
      </div>
      ...
      {course.resource_url && (
        <a
          ...
          className="inline-block mt-2 text-xs text-indigo-600 dark:text-[#C9D0D6] hover:underline"
        >
          Visit course ↗
        </a>
      )}
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 dark:border-[#202026]">
        {isCompleted ? (
