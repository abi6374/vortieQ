...
export default function ConnectGitHubModal({ isOpen, onClose, onConnected, onRemindLater }) {
  ...
  const handleSyncUsername = async (e) => {
    e?.preventDefault()
    const cleanHandle = username.trim().replace(/^https?:\/\/(www\.)?github\.com\//i, '').replace(/^@/, '').replace(/\/$/, '')
    if (!cleanHandle) {
      ...
      <div className="mt-4 pt-3.5 border-t border-[#CBD5E1]/60 dark:border-[#27272F] flex flex-wrap items-center justify-between gap-3 text-xs">
        {syncedUser ? (
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            >
              <svg className="w-3.5 h-3.5 text-[#0066cc] dark:text-[#38BDF8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
